...
function formatNetworkDirectory(options) {
  return path.join(options.working_directory, "networks", "" + options.network_id);
};
  ...
  run: function (options, done) {
      ...
      Environment.fork(config, function(err) {
        ...
        temp.mkdir('migrate-dry-run-', function(err, temporaryDirectory) {
          ...
          copy(config.contracts_build_directory, temporaryDirectory, function(err) {
            ...
            config.artifactor = new Artifactor(temporaryDirectory, config);
    ...
    Contracts.compile(config, function(err) {
      ...
      Environment.detect(config, function(err) {
        config.network_directory = formatNetworkDirectory(config)
        config.artifactor = new Artifactor(config.network_directory, config)
