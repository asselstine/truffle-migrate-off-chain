...
function formatNetworkVersionPath(options) {
  return path.join(options.network_directory, "version");
};
...
Migration.prototype.run = function(options, callback) {
  ...
  var finish = function(err) {
    ...
    deployer.start().then(function() {
      ...
      // write version
      mkdirp(options.network_directory, function (err) {
          if (err) console.error(err)
          else {
            fs.writeFileSync(formatNetworkVersionPath(options), self.number, { flags: 'w' });
          }
      });
    }).then(function() {
      if (options.save === false) return;
...
var Migrate = {
  ...
  lastCompletedMigration: function(options, callback) {
    var networkVersionPath = formatNetworkVersionPath(options);

    if (fs.existsSync(networkVersionPath) == false) {
      var networkVersion = 0;
    } else {
      networkVersion = parseInt(fs.readFileSync(networkVersionPath));
    }
