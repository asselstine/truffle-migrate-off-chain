...
function updateLatestContracts(options) {
  ...
  fs.readdirSync(options.networks_directory).forEach(function (file) {
    var json = readNetworkJSON(path.join(options.networks_directory, file))
    var mostRecentContractByNameOnNetwork = {}

    // the last contract will be the one recorded
    json.contracts.forEach(function (contract) {
      mostRecentContractByNameOnNetwork[contract.contractName] = contract;
    })

    var networkId = path.basename(file, '.json')

    _.keys(mostRecentContractByNameOnNetwork).forEach(function (contractName) {
      var networkContract = mostRecentContractByNameOnNetwork[contractName]
      var existingContract = latestContracts[contractName] || _.omit(networkContract, ['address'])
      existingContract.networks = existingContract.networks || {}
      existingContract.networks[networkId] = {
        events: {},
        links: {},
        address: networkContract.address
      }
      latestContracts[contractName] = existingContract
    })
  })

  logger.log()
  _.keys(latestContracts).forEach(function (contractName) {
    var contractPath = path.join(options.contracts_build_directory, '' + contractName + '.latest.json')
    logger.log('Updating latest contract ' + contractPath + '...')
    fs.writeFileSync(contractPath, JSON.stringify(latestContracts[contractName], null, 2))
  })
}
...
Migration.prototype.run = function(options, callback) {
  ...
  var finish = function(err) {
    ...
    deployer.start().then(function() {
      ...
      logger.log("Storing deployed contract addresses...");
...
var Migrate = {
  ...
  updateLatestContracts: function (options) {
    updateLatestContracts(options);
  },
